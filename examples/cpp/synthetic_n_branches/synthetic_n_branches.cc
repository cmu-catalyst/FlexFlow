...
#include "synthetic_n_branches.h"

using namespace Legion;

LegionRuntime::Logger::Category log_app("synthetic_n_branches");
...
SyntheticConfig::SyntheticConfig(void) {
...
void FlexFlow::top_level_task(Task const *task,
 ...
 // output = ff.dense(output, tfConfig.tt_hidden_size, AC_MODE_NONE, false /*bias*/);
 ...
 log_app.print("Num. iterations/epoch = %d", 12);
 ...
 printf("ELAPSED TIME = %.4fs, THROUGHPUT = %.2f samples/s\n",
        run_time,
        1 * ffConfig.epochs / run_time);
}

DataLoader::DataLoader(FFModel &ff,
                      SyntheticConfig const &tf,
